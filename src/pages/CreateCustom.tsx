...
<script>${jsCode}<\/script>
  ...
  const mobileView = (
    <div className="lg:hidden min-h-screen bg-app-bg dark:bg-dark-bg">
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3">
        <button type="button" onClick={() => navigate(-1)} className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
          <ArrowLeft className="h-5 w-5 text-gray-700 dark:text-gray-300" />
        </button>
        <h1 className="text-base font-bold text-gray-900 dark:text-gray-100">Custom Poll</h1>
        <div className="w-10" />
      </header>

      <div className="px-4 py-4 pb-24 space-y-6">
        <div>
          <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Poll Title</label>
          <input
            type="text"
            value={question}
            onChange={(e) => setQuestion(e.target.value)}
            placeholder="What's the topic of this poll?"
            className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 px-4 py-3 text-sm outline-none focus:border-primary-400"
          />
        ...
        {/* Options list */}
        <div>
          <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Options ({options.length})</label>
            ...
            {options.map((opt, idx) => (
              ...
              >
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-sm font-medium text-gray-800 dark:text-gray-200 outline-none min-w-0"
                />
                <button
                  type="button"
        ...
        {/* Mobile preview */}
        <div>
          <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Preview: {activeOption.name}</label>
        ...
        {/* Mobile settings */}
        <div>
          <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Settings</label>
          <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <div className="px-4 py-4">
              <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">Duration</p>
      ...
      <div className="lg:ml-64 min-h-screen flex flex-col">
        ...
        <header className="flex items-center justify-between px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Custom Poll Editor</h1>
        ...
        {showConfig && (
          <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
            <div className="max-w-4xl mx-auto grid grid-cols-4 gap-6">
              <div>
                <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Poll Title / Question</label>
                <input
                  type="text"
                  value={question}
                  onChange={(e) => setQuestion(e.target.value)}
                  placeholder="What's the topic of this poll?"
                  className="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 px-4 py-2.5 text-sm outline-none focus:border-primary-400"
                />
                {errors.question && <p className="mt-1 text-xs text-red-500">{errors.question}</p>}
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">Duration</label>
                <div className="flex flex-wrap gap-2">
                  {DURATION_OPTIONS.map((opt) => (
                    <button
                      key={opt.value}
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-56 shrink-0 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-y-auto">
            <div className="px-4 py-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100">Options</h3>
                <button
                  type="button"
                  onClick={addOption}
                  className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400 hover:bg-primary-100 dark:hover:bg-primary-900/30"
                  title="Add option"
                >
                ...
                {SNIPPETS.map((snippet) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-3 rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-3 text-left hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:border-primary-200 transition-colors"
                    >
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-100 dark:bg-primary-900/30 shrink-0">
                        <Icon className="h-4 w-4 text-primary-600 dark:text-primary-400" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{snippet.label}</p>
          ...
          <div className="w-80 shrink-0 border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">Preview: {activeOption.name}</span>
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => setPreviewMode('desktop')}
                  className={`p-1.5 rounded-lg transition-colors ${previewMode === 'desktop' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
                >
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-colors ${previewMode === 'mobile' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
                >
            ...
            <div className="flex-1 overflow-auto bg-gray-50 dark:bg-gray-800 p-4">
              <div className={`mx-auto h-full ${previewMode === 'mobile' ? 'max-w-[320px]' : ''}`}>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-2 font-medium">Editing: {activeOption.name}</p>
                <iframe
                  key={`${previewKey}-${activeOption.id}`}
                  srcDoc={previewSrcDoc}
                  title="Live Preview"
            ...
            {/* All options preview */}
            <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-3 bg-gray-50 dark:bg-gray-800 overflow-y-auto" style={{ maxHeight: '250px' }}>
                ...
                {options.map((opt, idx) => (
                  <div
                    ...
                    className={`rounded-lg border overflow-hidden cursor-pointer transition-colors ${
                      idx === activeOptionIdx ? 'border-primary-400 ring-1 ring-primary-200 dark:ring-primary-800' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                    }`}
                  >
                    <iframe
                    ...
                    <div className="px-2 py-1 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700">
                      <p className="text-xs font-medium text-gray-600 dark:text-gray-400 truncate">{opt.name}</p>
                    </div>
                  </div>
                ))}
              </div>
